...
- **提交人与复核人不能是同一人**：操作员需登录（账号密码），身份以服务端会话为准，前端不能自报姓名
- 修复放行生效前强校验：尚有未结工单不放行；存在「继续隔离」结论的工单不放行（如需出库请走退回流程）
...
工单办理（拦截登记、认领、转交、补充证据、提交结论、复核）需要先登录，演示账号（密码均为 `123456`）：
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/auth/login` `/logout` | 登录 / 注销（签发服务端会话 token） |
| GET | `/api/auth/me` `/api/auth/users` | 当前会话用户 / 用户列表 |
