...
### 6. 分拣规则与格口
- **格口维护**：新增格口、启用/停用；停用时可选择**改道**——变更写入规则草稿（不直接改写已发布版本），试算发布后生效，停用期间扫描落空进待判区
- **规则版本化**：按「目的地 + 重量区间 + 优先级」配置规则，调整走 **草稿 → 试算 → 发布** 流程；**未试算不得发布**，规则或格口状态（停用/启用）任何变更后都需重新试算
- **试算影响**：发布前用当前全部待分拣包裹模拟，输出自动路由覆盖率、规则冲突数、无匹配数、各格口预计负荷与规则命中排行
- **扫描分拣**：扫描运单即返回**目标格口 + 命中原因 + 规则版本**；规则冲突（同级多规则命中）或无匹配时**当场转入待判区**，人工指定格口后出库
- **整车分拣**：车辆「完成分拣」逐件按发布版规则路由并写入格口快照，路由失败的进待判区；**未分配格口的包裹禁止装车**（含发车自动装车）
